...
import { KeyvStore } from "@apiser/controller/cache/keyv";
...
class MemoryStore implements CacheStore {
  ...
  async set<TValue = unknown>(key: string, value: TValue, options?: CacheCallbackOptions): Promise<void> {
...
describe("@apiser/controller", () => {
  describe("cache", () => {
    test("wrapHandler caches full handler callback result", async () => {
      ...
      const options = createOptions({
        ...
        cache: {
          store,
          wrapHandler: true,
          key: ({ payload }) => ["wrap", String((payload as { id: number }).id)],
        },
      });

      const handler = createHandler(options);
      const main = handler(async ({ payload }) => {
        calls += 1;
    ...
    test("cache() returns cached value on subsequent calls", async () => {
      ...
      const main = handler(async ({ cache, payload }) => {
