...
describe("@apiser/controller model binding", () => {
  ...
  test("merges where clause with primary key", async () => {
    ...
    const expected = await userModel.where({
      id: { eq: 1 },
      isVerified: { eq: false },
    }).findMany();
  ...
  test("load + notFound returns error", async () => {
    const handler = makeHandler({
      ...
      fromKey: "user",
      load: () => null,
      notFound: ({ fail }: { fail: (name: string, input?: unknown) => any }) => {
        throw fail("notFound");
      }
    });
