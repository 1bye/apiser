...
export type BindingPayload<
	TSchema extends Schema | undefined = Schema | undefined,
> = TSchema;
...
export type BindingInjectContext<
	TPayloadSchema extends Schema | undefined = Schema | undefined,
	THandler = unknown,
> = {
	payload: InferBindingPayload<TPayloadSchema>;
...
> = {
	...
	mode?: TMode;
	inject?: (
		ctx: BindingInjectContext<TPayloadSchema, THandler>
	) => PromiseOr<boolean>;
	payload?: TPayloadSchema;
	resolve: (
		ctx: BindingResolveContext<TPayloadSchema, THandler, TResponseHandler>
	) => Promise<TResult>;
};
...
> {
	 ...
	 */
	model<TModel extends ModelIdentifier>(
		model: TModel,
		_options?:
			| BindingModelOptions<TModel, TResponseHandler>
			| BindingModelOptions<TModel, TResponseHandler>[]
	): BindingFactory<
		[boolean],
		Schema,
...
export type BindingsWithNames<TBindings> = {
	[TKey in keyof TBindings]: TKey extends string
		? TBindings[TKey] extends BindingDefinition<
 ...
 */
export type BindingsFactory<TBindings> = (
	bindings: BindingsHelpers
) => TBindings;

/**
 * Bindings can be provided as an object or a factory.
 */
export type BindingsInput<TBindings> = BindingsWithNames<TBindings>;
// | BindingsFactory<BindingsWithNames<TBindings>>;

/**
 * Resolve bindings from a factory or object.
 */
export type ResolveBindings<TBindings> = TBindings extends (
	...args: any[]
) => infer TResult
	? TResult
	: TBindings;
 ...
 */
export type HandlerBindings<TOptions extends HandlerOptions<any, any, any>> =
	TOptions["bindings"] extends undefined
		? {}
	...
	model: (model, options) => {
		...
		return () => ({
			resolve: async ({ request, fail }) => {
				...
				let hasWhere = false;
				let loadHandler:
					| BindingModelOptions<ModelIdentifier, AnyResponseHandler>["load"]
					| undefined;
				let notFoundHandler:
					| BindingModelOptions<ModelIdentifier, AnyResponseHandler>["notFound"]
					| undefined;
				...
				for (const option of normalizedOptions) {
					...
					const fromKey =
						option?.fromKey ??
						(typeof defaultFromKey === "string" ? defaultFromKey : primaryKey);
					const source = sources[from] ?? {};
					const rawValue =
						typeof fromKey === "string"
							? getNestedValue(source, fromKey)
							: undefined;
					...
					if (option?.load) {
						loadHandler = option.load;
					}
					if (option?.notFound) {
						notFoundHandler = option.notFound;
					}
				...
				if (
					hasWhere &&
					combinedWhere &&
					typeof (resolvedModel as { where?: (value: unknown) => unknown })
						.where === "function"
				) {
					resolvedModel = (
						resolvedModel as { where: (value: unknown) => unknown }
					).where(combinedWhere);
				}
