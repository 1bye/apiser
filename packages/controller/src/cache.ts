...
export type CacheKeyResolver<TPayload = unknown> = (data: CacheKeyResolverData<TPayload>) => CacheKey;

export type CacheKeyInput<TPayload = unknown> = CacheKey | CacheKeyResolver<TPayload>;
...
export interface CacheStore {
  ...
  set<TValue = unknown>(key: string, value: TValue, options?: CacheCallbackOptions): PromiseOr<void>;
  delete?(key: string): PromiseOr<boolean | void>;
}

export interface CacheOptions<TPayload = unknown, TStore extends CacheStore = CacheStore> {
  wrapHandler?: boolean;
  store: TStore;
...
export type CreateCacheOptions<TPayload = unknown, TStore extends CacheStore = CacheStore> = {
  payload: TPayload;
...
export const resolveCacheOptions = <TPayload, TStore extends CacheStore>(input: {
  handlerCache?: CacheOptions<TPayload, TStore>;
  callCache?: CacheOptions<TPayload, TStore>;
  ...
  if (!merged.store) return undefined;

  return merged;
};

export const createCache = <TPayload, TStore extends CacheStore>(opts: CreateCacheOptions<TPayload, TStore>): CacheFn => {
