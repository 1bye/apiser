...
>(data: TRawData): Registry.Collection<TTags> {
	const _data = typeof data === "function" ? data(new BaseTag()) : data;

	const collection: Omit<Registry.Collection<TTags>, "derive"> = <
		TKey extends keyof TTags,
	>(
		name: TKey
	) => {
	...
	// @ts-expect-error
	collection.prototype.derive = (name, callback) => {
